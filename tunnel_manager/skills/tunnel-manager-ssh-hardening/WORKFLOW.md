...
Initial key setup needs a working key/agent credential or an opaque
`password_ref`. Independently verified host keys must already be present in the
governed `known_hosts` file.
...
| `TUNNEL_PASSWORD_REF` | optional | Runtime secret reference used only at connection time |
| `TUNNEL_KNOWN_HOSTS` | recommended | Independently verified server host keys |
...
```json
{"action": "configure_key_auth", "group": "managed", "key": "~/.ssh/id_shared", "key_type": "ed25519"}
```
Bootstrap a full-mesh trust (every host trusts every other):
```json
{"action": "mesh_bootstrap", "group": "managed", "parallel": true, "max_threads": 6}
```
Rotate the fleet key:
```json
{"action": "rotate_key", "group": "managed", "new_key": "~/.ssh/id_shared_next", "key_type": "ed25519"}
```
CIS compliance check against one host:
```json
{"action": "compliance_check", "remote_host": "managed-node", "username": "operator", "identity_file": "~/.ssh/id_ed25519", "standard": "cis_benchmark"}
```

## Gotchas
- Key setup / mesh must bootstrap from an existing credential. Passwords are
  accepted only through `password_ref`, never as MCP values or inventory fields.
- Mesh distributes authentication keys only. It never populates `known_hosts`
  through unauthenticated scans; peer host-key trust must be pre-provisioned.
