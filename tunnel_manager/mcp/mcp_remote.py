    ...
    async def tm_remote(
        ...
        if action == "run_command":
            ...
            return await _tm_remote_check_ssh(
                host, user, password, port, id_file, certificate, proxy, cfg, ctx
            )
        elif action == "test_key_auth":
            return await _tm_remote_test_key_auth(host, user, port, cfg, key, ctx)
        elif action == "setup_passwordless":
            return await _tm_remote_setup_passwordless(
                host, user, password, port, cfg, key, key_type, ctx
            )
